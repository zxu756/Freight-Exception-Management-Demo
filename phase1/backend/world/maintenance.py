...
def escalate_stale_approvals(db, now):
    """APR-005 审批超时升级：pending_approval 超过 24 模拟小时未处理 → 升级（escalated）。"""
    from sea_freight_models import SeaException
    from air_cargo_models import AirException
    from road_freight_models import RoadException
    from rail_freight_models import RailException
    ...
    upgraded = 0
    for cls in (SeaException, AirException, RoadException, RailException):
        rows = db.query(cls).filter(
            cls.status == "pending_approval",
            cls.detected_at <= threshold,
        ).all()
        for exc in rows:
            exc.status = "escalated"
            exc.escalation_reason = f"审批超时升级：{exc.exception_id} 待审批超过 24 小时（检测于 {exc.detected_at:%m-%d %H:%M}）"
            upgraded += 1
    if upgraded:
        db.commit()
    return upgraded
...
def maintenance(db, now):
    ...
    upgraded = escalate_stale_approvals(db, now)
    n = snapshot_metrics(db, now)
    cutoff = now - timedelta(days=60)
    db.query(MetricSnapshot).filter(MetricSnapshot.ts < cutoff).delete()
    db.commit()
    print(f"[world] maintenance @ {now:%m-%d %H:%M}: carrier perf refreshed, {n} metric snapshots, {upgraded} approvals escalated")
    return n
